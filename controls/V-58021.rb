...
addressed, and must document what has been discovered.
  ...
  desc  'rationale', ''
  desc  'check', "
    Review the system documentation, database and DBMS security configuration,
source code for DBMS internal logic, source code of external modules invoked by
the DBMS, and source code of the application(s) using the database.
...
external modules invoked by the DBMS, and the application(s) using the
database, to ensure privilege elevation is used only as required.
  "
  impact 0.5
  tag severity: 'medium'
