...
control "V-61407" do
  ...
  describe json({ command "couchbase-cli setting-password-policy -c #{input('cb_cluster_host')}:#{input('cb_cluster_port')} -u #{input('cb_full_admin')} -p \
  #{input('cb_full_admin_password')} --set --min-length 15 --uppercase --lowercase --digit --special-char"} ) do
