	...
	tag "nist": ["AU-3 (2)", "Rev_4"]
  	...
  	if couchbase_version >= '6.5.1'
    	describe command("couchbase-cli setting-audit -u #{input('cb_full_admin')} -p \
    	#{input('cb_full_admin_password')} --cluster #{input('cb_cluster_host')}:#{input('cb_cluster_port')} \
    	--get-settings | grep 'Audit enabled:'") do
