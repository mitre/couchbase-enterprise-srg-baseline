...
exist to protect the DBMS's private key(s). If the DMBS’s private key(s) are
not stored in a FIPS 140-2 validated cryptographic module, this is a finding.

    If access to the DBMS’s private key(s) is not restricted to authenticated
and authorized users, this is a finding.
  "
  desc  'fix', "Store all DBMS PKI private keys in a FIPS 140-2 validated
cryptographic module.  Ensure access to the DBMS PKI private keys is restricted
